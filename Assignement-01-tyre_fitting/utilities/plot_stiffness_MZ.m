...
subplot(2,1,1)
hold on
plot(mean(FZ_220.FZ),Calfa_vec1_0,'+','LineWidth',2, 'Color', colors_vect(2,:))
plot(mean(FZ_440.FZ),Calfa_vec2_0,'+','LineWidth',2, 'Color', colors_vect(3,:))
plot(mean(FZ_700.FZ),Calfa_vec3_0,'+','LineWidth',2, 'Color', colors_vect(4,:))
plot(mean(FZ_900.FZ),Calfa_vec4_0,'+','LineWidth',2, 'Color', colors_vect(5,:))
plot(mean(FZ_1120.FZ),Calfa_vec5_0,'+','LineWidth',2, 'Color', colors_vect(6,:))
legend({'$Fz_{220}$', '$Fz_{440}$', '$Fz_{700}$','$Fz_{900}$','$Fz_{1120}$'}, 'Location', 'best')
xlabel('Vertical load [N]')
ylabel('$C_{\alpha, MAX}$ TO CHECK [N/rad]')
grid on
title('Longitudinal cornering stiffness for $\alpha = 0$ TO CHECK')
