...
if strcmp(load_type, "longitudinal")
  [SL, order] = sort(SL(smpl_range)); % sort by SA
  SA = SA(order);
elseif (strcmp(load_type, "lateral") || strcmp(load_type, "self_aligning"))
  [SA, order] = sort(SA(smpl_range)); % sort by SA
  SL = SL(order);
else 
  error("load_type must be either 'longitudinal', 'lateral' or 'self_aligning'");
end
FX = FX(order);
FY = FY(order);
MZ = MZ(order);
